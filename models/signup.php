...
if ($_POST['password'] == $_POST['confirmpassword']) {
						...
						if ($res = $req->rowCount() > 0 ) {
							...
							if ($res = $req->rowCount() > 0 ) {
								...
								if ( preg_match("/^[A-Za-z0-9]+(?:\.[A-Za-z0-9]+)*@[A-Za-z0-9]+(?:\.[A-Za-z0-9]+)*(?:\.[A-Za-z]{2,})$/", $_POST['email']) ) {
									...
									$req = $db->prepare("INSERT INTO tb_users(nom, phone, email, avatar, password) VALUES (?,?,?,?,?) ");
									$pwd = hash('sha512', $_POST['password']);
									$req->execute([ $_POST['username'], $_POST['phone'], $_POST['email'], $avatar, $pwd ]);
	...
	$_SESSION['error'] = "Le deux mot de passe doivent être identique";
	exit();
}